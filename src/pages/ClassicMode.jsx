...
export default function ClassicMode() {
  ...
  const checkAnswer = () => {
    ...
    const isInputCorrect = allAnimeTitles.some((anime) =>
      anime.titles.some(
        (title) => title.title.toLowerCase() === inputText.trim().toLowerCase(),
      ),
    );
    for (const element of alreadyInserted) {
    ...
    sessionStorage.setItem(
      "alreadyInserted",
      JSON.stringify([...alreadyInserted, inputText]),
    );
  };

  const getRandomAnime = () => {
    const englishTitles = allAnimeTitles.filter((anime) =>
    ...
    const randomTitle = randomAnime.titles.find(
  ...
  return (
    <div className="App">
      ...
      <div className="title__container">
        {!isAnswerWrong && <h3>QUAL O ANIME DA CAPA?</h3>}
        {isAnswerWrong && <h3 className="title_game_over">VOCÊ PERDEU!</h3>}
      </div>
      <div className="img__container">
        <div
          className={`blur ${pixelSize < 6 && remainingAttempts === 1 ? "strong-blur" : ""}`}
        >
          {animeCover && (
            <ImagePixelated
              width={300}
      ...
      <div className="if__correct">
        ...
        {(isAnswerCorrect || isAnswerWrong) && (
          <button className="next_button" onClick={getRandomAnime}>
            {isAnswerCorrect ? "Próximo" : "Jogar Novamente"}
          </button>
        )}
      </div>
      {!(isAnswerCorrect || isAnswerWrong) && (
        <div className="search__container" ref={inputRef}>
          ...
          <div className="btn__container">
            <button type="button" className="btn__pass" onClick={dontKnow}>
              NÃO SEI
            </button>
            <button type="button" className="btn__check" onClick={checkAnswer}>
              ADIVINHAR 推測
            </button>
          </div>
        </div>
              ...
              <tbody>
                {alreadyInserted.map((insertedName, index) => {
                  const anime = allAnimeTitles.find((anime) => {
                    return anime.titles.some(
                      (title) => title.title === insertedName,
                    );
                  });
                  if (anime) {
                    const correctYear = correctAnime.aired.string
                      .split(" to ")[0]
                      .split(",")[1]
                      .trim();
                    const year = anime.aired.string
                      .split(" to ")[0]
                      .split(",")[1]
                      .trim();
                    return (
                      <tr key={index} className="inserted__info">
                        <td
                          className={`table-cell ${correctAnime.title === anime.title ? "correct-cell" : "incorrect-cell"}`}
                        >
                          {anime.title}
                        </td>
                        <td
                          className={`table-cell ${correctAnime.type === anime.type ? "correct-cell" : "incorrect-cell"}`}
                        >
                          {anime.type}
                        </td>
                        <td
                          className={`table-cell ${correctAnime.status === anime.status ? "correct-cell" : "incorrect-cell"}`}
                        >
                          {anime.status}
                        </td>
                        <td
                          className={`table-cell ${correctYear === year ? "correct-cell" : "incorrect-cell"}`}
                        >
                          {correctYear > year
                            ? year + "▸"
                        ...
                        <td
                          className={`table-cell ${correctAnime.studios[0].name === anime.studios[0].name ? "correct-cell" : "incorrect-cell"}`}
                        >
